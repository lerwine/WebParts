...
namespace MenuTree
{
	class CMTToolPart : ToolPart, System.Web.UI.INamingContainer
	{
		...
		private void MenuTreeToolPart_Init(object sender, EventArgs e)
		{
			...
			this.xmlDocMenuData = new System.Xml.XmlDocument();
			if ((strXmlData = ((MenuTree)(this.ParentToolPane.SelectedWebPart)).MenuData.Trim()) == "")
			{
				node = this.xmlDocMenuData.CreateNode("element", "menu", "");
				this.xmlDocMenuData.AppendChild(node);
			}
			else
			{
				this.xmlDocMenuData.LoadXml(strXmlData);
			}
		...
		private string GetXmlData()
		{
			...
			try
			{
				...
				this.xmlDocMenuData.Save((System.IO.Stream)ms);
				buffer = ms.ToArray();

				strXmlData = System.Text.Encoding.UTF8.GetString(buffer);
			}
			finally
			{
				if (ms != null) ms.Close();
		...
		{
			return new object[] { this.strEditingNode, this.GetXmlData(), this.nDeletePosition, base.SaveViewState() };
		}
		...
		protected override void LoadViewState(object savedState)
		{
			...
			this.xmlDocMenuData.LoadXml(Convert.ToString(((object[])(savedState))[1]).Trim());
			this.editingNode = this.xmlDocMenuData.SelectSingleNode(this.strEditingNode);
		...
		{
			((MenuTree)(this.ParentToolPane.SelectedWebPart)).MenuData = this.GetXmlData();
		}
