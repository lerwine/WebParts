...
function WPQ3mouseClick() {
    ...
    if (!ulNode.style || !ulNode.style.display || ulNode.style.display != "block") {
        ...
        ulNode.style.top = 0;
        ulNode.style.left = ulNode.parentNode.parentNode.offsetWidth - 6;
        for (node = ulNode.parentNode.firstChild; node != null; node=node.nextSibling) {
            if (node.nodeType == 1 && node.className == "ExpandImage")
                node.src = "http://spsdev/sites/wptest/_layouts/images/TPMIN.GIF";
        }
    } else {
        WPQ3hideNodeTree(ulNode);
        for (node = ulNode.parentNode.firstChild; node != null; node=node.nextSibling) {
            if (node.nodeType == 1 && node.className == "ExpandImage")
                node.src = "http://spsdev/sites/wptest/_layouts/images/TPMAX.GIF";
        }
    }
}
...
function WPQ3mouseOver() {
    ...
    currentNode.style.top = currentNode.parentNode.offsetTop + 1;
